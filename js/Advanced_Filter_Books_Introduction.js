...
$(document).ready(async function () {
  ...
  resourceISName.innerHTML = IS_Name
    ? IS_Name
    : (document.querySelector("#resource-is").style.display = "none");
  resourceJSName.innerHTML =
    JC_Name || type
      ? JC_Name || type
      : (document.querySelector("#resource-js").style.display = "none");
  resourceOBName.innerHTML = OB_Name
    ? OB_Name.split(",").join("/")
    : (document.querySelector("#resource-ob").style.display = "none");
  resourceCSName.innerHTML = CS_Name
    ? CS_Name.split(",").join("/")
    : (document.querySelector("#resource-cs").style.display = "none");
  resourceCRName.innerHTML = CR_Name
    ? CR_Name.split(",").join("/")
  ...
  $("#resource-download").click(async function () {
    ...
    await downloadResource(bookId, detailResource);
...
function share_fb() {
  ...
  var share_link =
    "https://www.facebook.com/sharer/sharer.php?u=" + window_location_encoded;

  if (/Android/i.test(navigator.userAgent)) {
    // Open in Facebook app on Android devices
    window.location.href =
      "intent://share/#Intent;scheme=fb;action=android.intent.action.SEND;type=text/plain;S.com.facebook.katana.extra.APPLICATION_ID=com.facebook.katana;B.android.intent.extra.TEXT=" +
      window_location_encoded +
      ";end";
...
function share_twitter() {
  var window_location_encoded = encodeURIComponent(window.location.href);
  var share_link =
    "https://twitter.com/intent/tweet?url=" + window_location_encoded;
