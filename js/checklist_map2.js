...
function checkIsAllSelect() {
  subLevelMainList.forEach((id) => {
    ...
    const subCheckboxs = subSearchItem.querySelectorAll(
      'input[type="checkbox"]'
    );
  ...
  for (let i = 2; i <= 3; i++) {
    ...
    const checkboxs = searchItemElement.querySelectorAll(
      'input[type="checkbox"]'
    );
  ...
  switch (level) {
    case 1:
      checkboxes.forEach(function (checkbox) {
        if (checkbox.checked) {
          const searchMainMenu = document.querySelector(
            `.searchitem-${blockId} > .sub-searchmenumain`
          );
    ...
    case 2:
      checkboxes.forEach(function (checkbox) {
        if (checkbox.checked) {
          const searchMainMenu = document.querySelector(
            `.sub-searchitem-${blockId} > .sub-searchmenusub`
          );
          searchMainMenu.style.display = "block";
          const mainDropdown = document.querySelector(
            `.sub-dropdown-${blockId}`
          );
          mainDropdown.classList.remove("rotatebefore");
          mainDropdown.classList.add("rotateafter");
        }
...
function initChecklist() {
  ...
  subLevelMainList.forEach((subLevelNumber) => {
    const subSearchItemElement = document.querySelector(
      `.sub-searchitem-${subLevelNumber}`
    );
...
$(document).ready(async function () {
  ...
  const filterIds =
    queryString && queryString.filterId
      ? queryString.filterId.split(",")
      : [""];
  ...
  $(".sub-btn").click(function () {
    var subsearchmenu = $(this)
      .closest(".searchitem")
      .find(".sub-searchmenusub");
  ...
  $(".searchitem").each(function () {
    ...
    subCheckboxes.click(function () {
      ...
      var parentCheckbox = $(this)
        .closest(".searchitemmain")
        .find(".searchmenu-checkbox");
      var childCheckboxes = $(this)
        .closest(".sub-searchmenu")
        .find(".child-sub-checkbox");
      if (!isChecked) {
    ...
    bigsubCheckboxes.click(function () {
      var isChecked = $(this).is(":checked");
      var parentCheckbox1 = $(this)
        .closest(".searchitemmain")
        .find(".searchmenu-checkbox");
      var grandparentCheckbox = $(this)
        .closest(".searchitemmain")
        .find(".searchmenu-checkbox");
      var childCheckboxes = $(this)
        .closest(".searchitemsub")
        .find(".child-sub-checkbox");
      childCheckboxes.prop("checked", isChecked);
      initChecklist();
      if (!isChecked) {
  ...
  $(".searchmenu-checkboxall").click(function () {
    var isChecked = $(this).is(":checked");
    $(
      ".searchmenu-checkbox, .sub-checkbox, .bigsub-checkbox, .child-sub-checkbox"
